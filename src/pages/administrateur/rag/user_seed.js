...
export default function User_seed() {
    ...
    const fetchRecipes = async () => {
        ...
        try {
            ...
            const recipesList = recipeData.data.listProfiles.items;
                ...
                try {
                    ...
                    const { text } = await generateText({
                        ...
                        messages: [
                            {
                                role: 'system',
                                content: `Tu es dans une application de recette qui consiste à mettre en avant la communauté, et les traditions culinaires durant le ramadan. Ton rôle est de résumer un profil utilisateur pour qu'il soit optimisé pour du rag, dont sa description, si c'est précisé : ses attentes de l'application, ce qu'il préfère en cuisine, ce qu'il n'aime pas, ses allergies sa date de naissance sans autres commentaires.`,
                            },
                            {
                                role: 'user',
    ...
    const insertVectorToPinecone = async (embeddingVector, id) => {
        try {
            await index.namespace("profile").upsert([
                {
                    id: id.toString(),
