...
import { ingredientsByTypeID, listTags } from "@/graphql/queries";
...
function TypesIngredientsPage() {
    const [tags, setIngredients] = useState([]);
    ...
    const fetchData = async () => {
        try {
            ...
            setIngredients(result.data.listTags.items);
            setLoading(false);
        } catch (error) {
    ...
    const handleChildCheckboxChange = (ingredientId) => {
        ...
        setAllChecked(count === tags.length);
    };
    ...
    const handleParentCheckboxChange = () => {
        ...
        if (!allChecked) {
            tags.forEach((tag) => {
                updatedCheckedItems[tag.id] = true;
            });
        }
    ...
    const deleteCheckedIngredients = async () => {
        ...
        try {
            ...
            console.error("Error on deleting tags", error);
    ...
    const handleDeleteIngredientType = async (ingredientTypeId) => {
        try {
            ...
            console.error("Error on deleting tag and its ingredients", error);
                            ...
                            tags.map((tag) => (
                                <tr key={tag.id}>
                                    {isAdmin && (
                                        <Td>
                                            <Checkbox
                                                checked={checkedItems[tag.id] || false}
                                                onChange={() => handleChildCheckboxChange(tag.id)}
                                            />
                                        </Td>
                                    )}
                                    <Td>
                                    ...
                                    {isAdmin && (
                                        <Td>
                                            <Stack spacing="10px">
                                                <IconButton onClick={() => handleDeleteIngredientType(tag.id)} variant="danger">
                                                    <CiTrash />
                                                </IconButton>
                                            </Stack>
                                        </Td>
